...
public final class Settings {
    ...
    public static void loadSettingsFile() {
        File settingsFile = new File(new File(IDDefs.JAR_POSITION).getParent() + File.separator + Defs.Resources.JSON.CLIENT_SETTINGS);
        if (!settingsFile.exists() || !settingsFile.isFile()) {
            try {
                JarHandler.copyFileFromJar(new File(IDDefs.JAR_POSITION), Defs.Resources.JSON.DEFAULT_JSON_SETTINGS, settingsFile, true);
            } catch (IOException e) {
    ...
    public static void writeJSONSettings() throws IOException {
        JSONManager.writeJSON(SETTINGS, new File(new File(IDDefs.JAR_POSITION).getParent() + File.separator + Defs.Resources.JSON.CLIENT_SETTINGS));
    }
